...
const Contact = () => {
  ...
  const onSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    const form = event.currentTarget; // ✔ Correct type
    const formData = new FormData(form); // ✔ No TS errors

    formData.append(
      "access_key",
      `${process.env.NEXT_PUBLIC_EMAIL_ACCESS_KEY}`
    );
    ...
    <section id="contact" className="section">
      <div className="container lg:grid lg:grid-cols-2  lg:items-stretch ">
        <div className="mb-12 lg:mb-0 lg:flex lg:flex-col ">
          <h2 className="text-3xl lg:text-4xl lg:leading-tight font-semibold lg:max-w-[12ch] ">
            Get in Touch
          </h2>
          ...
          <div className="flex  items-center gap-2 ">
            {socialLinks.map(({ href, icon, alt }, key) => (
              <Link
                className="w-12 h-12 grid place-content-center ring-inset ring-2 ring-zinc-50/5 rounded-lg transition-[background-color, color] hover:bg-zinc-50 hover:text-zinc-950 active:bg-zinc-50/80 "
                href={href}
                target="_blank"
                key={key}
              >
        ...
        <form onSubmit={onSubmit} method="POST" className="xl:pl-10 2xl:pl-20">
          ...
          <div className="mb-4">
            ...
            <textarea
              ...
              placeholder="Hi"
              required
              className="text-field resize-y min-h-60 md:m-h-40 max-h-80 "
            ></textarea>
          </div>
